...
class MainMenuState(BaseState):
    ...
    def enter(self) -> None:
        ...
        if self.game.has_saved_game():
            items.append(("Continuar partida", self.game.continue_game))
        ...
        self.menu = Menu(self.game, items, x=44, y=198, width=224,
                         height=36, spacing=40)
    ...
    def render(self, surface) -> None:
        ...
        surface.blit(label, label.get_rect(center=(self.panel.centerx, 161)))
        if self.game.game_save_error:
            status, color = "No se pudo cargar la partida", settings.PLACEMENT_INVALID_COLOR
        elif self.game.tutorial_progress_error:
        ...
        surface.blit(label, label.get_rect(center=(self.panel.centerx, 180)))
